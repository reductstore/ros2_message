...
- Workspace root hosts the `ros2_message` crate (library). Shared config lives in the top-level `Cargo.toml`.
...
- Tests: unit tests colocated under `ros2_message/src/tests` and `src/parse_msg/tests.rs`; sample assets under `msg_examples/`.

## Build, Test, and Development Commands
- Format: `cargo fmt --all` (run before commits).
- Lint: `cargo clippy --workspace --all-targets --all-features` (fail on warnings).
- Build: `cargo build --workspace --all-features` (currently just the `decode` feature).
...
- Prefer explicit feature gating (`decode`) and keep error handling via `Result`/`thiserror` consistent.
